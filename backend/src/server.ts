...
import zvanja from './model/zvanja';
import predmeti from './model/predmeti';
import zaposleni from './model/zaposleni';
import obavestenja from './model/obavestenja';
import studenti from './model/studenti';
import termini from './model/termini';
import admin from './model/admin';
import opsta_obavestenja from './model/opsta_obavestenja';
import projekti from './model/projekti';
import zahtevi from './model/zahtevi';
import spiskovi from './model/spiskovi';
...
app.use(express.static(__dirname + "/../podaci/slike_zaposlenih"))
app.use(express.static(__dirname+"/../podaci/predmeti/"))
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        for (let i = 0; i < (req as any).files.length; i++){
          console.log((req as any).files[i].filename)
          console.log((req as any).files[i].path)
        }

        console.log(req.body);
        console.log((req as any).files[0].filename)
        console.log((req as any).files[0].path)
      }
  });
});
...
app.post('/dodavanjeSlikeZaposlenog/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        console.log(req.body);
        console.log((req as any).file.filename)
        console.log((req as any).file.path)
        zaposleni.updateOne({ "korime": korime }, { "slika": (req as any).file.path}, (err, p) => {
          if (err) console.log(err)
          else console.log("OK JE SVE")
        })
...
app.post('/dodajNovoPredavanje/:sifra/:ime/:prezime/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        let predavanje = {
          "naziv_fajla": ((req as any).file.filename),
          "link": ((req as any)).file.path,
          "datum": Date.now(),
          "tip_fajla": ((req as any)).file.mimetype,
          "velicina_fajla": ((req as any)).file.size,
          "ime": ime,
          "prezime": prezime,
          "korime": korime
        }

        predmeti.updateOne({ "sifra": sifra }, {$push:{"predavanja":predavanje}}, (err, p) => {
          if (err) console.log(err)
          else {
...
app.post('/dodavanjeFajlovaObavestenje/:id/:sifra/:predmet', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        for (let i = 0; i < (req as any).files.length; i++){
          let fajl_upload = {
            "naziv_fajla": ((req as any).files[i].filename),
            "link": ((req as any)).files[i].path,
            "datum": Date.now(),
            "tip_fajla": ((req as any)).files[i].mimetype,
            "velicina_fajla": ((req as any)).files[i].size,
            "predmet": predmet,
            "originalni_naziv":((req as any)).files[i].originalname,
          }
          obavestenja.updateOne({ "_id": id }, { $push: { "fajlovi": fajl_upload } }, (err, res) => {
            if (err) console.log(err)
            else console.log("Uspesno ucitao dodatak za obavestenje!")
          })
        }
         res.end("File has been uploaded");
      }
...
app.post('/dodajNoveVezbe/:sifra/:ime/:prezime/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        predmeti.updateOne({ "sifra": sifra }, {$push:{"vezbe":vezbe}}, (err, p) => {
          if (err) console.log(err)
          else {
...
app.post('/dodajNoviIspit/:sifra/:ime/:prezime/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        predmeti.updateOne({ "sifra": sifra }, {$push:{"ispiti":ispit}}, (err, p) => {
          if (err) console.log(err)
          else {
            res.json({"poruka":"OK"})
...
app.post('/dodajNovuLaboratoriju/:sifra/:ime/:prezime/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        predmeti.updateOne({ "sifra": sifra }, {$push:{"labvezbe":lab}}, (err, p) => {
          if (err) console.log(err)
          else {
            res.json({"poruka":"OK"})
...
app.post('/dodajNovMaterijalProjekat/:sifra/:ime/:prezime/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        predmeti.updateOne({ "sifra": sifra }, {$push:{"projekat_materijali":projekat}}, (err, p) => {
          if (err) console.log(err)
          else {
            res.json({"poruka":"OK"})
...
app.post('/dodajNovMaterijalDomaci/:sifra/:ime/:prezime/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        predmeti.updateOne({ "sifra": sifra }, {$push:{"domaci_materijali":domaci}}, (err, p) => {
          if (err) console.log(err)
          else {
            res.json({"poruka":"OK"})
            //console.log("Uspesno ucitao materijal za projekat!")
...
app.post('/posaljiRad/:id/:sifra/:korime', function (req, res) {
  ...
  upload(req,res,function(err:any) {
      if(err) {
        ...
        let rad = {
          "korime":korime,
          "putanja": ((req as any)).file.path,
          "ime_fajla": (req as any).file.filename
        }

        let link = ((req as any)).file.path

        spiskovi.updateOne({ "_id": id }, {$push:{"prijavljeni":rad}}, (err, p) => {
          if (err) console.log(err)
          else console.log("Uspesno ucitao rad!")
        })
...
router.route('/dohvPredmete').post((req, res) => {
  ...
  predmeti.find({"tip_studija":tip_studija,"smer":smer}, (err, p) => {
    if (err) console.log(err)
    else res.json(p)
  })
})
...
router.route('/registrujZaposlenog').post((req, res) => {
  ...
  zaposleni.findOne({"korime": zaposlen.korime}, (err, k) => {
    if(err) console.log(err)
    else {
      /*
...
router.route('/registrujStudenta').post((req, res) => {
  ...
  studenti.findOne({"korime": student.korime}, (err, k) => {
    if(err) console.log(err)
    else {
      /*
...
router.route('/dohvatiZaposlenog').post((req, res) => {
  ...
  zaposleni.findOne({"korime": korime, "lozinka":lozinka}, (err, k) => {
    if(err) console.log(err)
    else {
      res.json(k)
    }
...
router.route('/azurirajInfoZaposleni').post((req, res) => {
  let zaposlen = req.body.zaposlen;
  zaposleni.updateOne({ "korime": zaposlen.korime }, { "ulica": zaposlen.ulica, "grad": zaposlen.grad, "mobilni": zaposlen.mobilni, "broj_kabineta": zaposlen.broj_kabineta, "licni_podaci": zaposlen.licni_podaci }, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/dodajObavestenje').post((req, res) => {
  ...
  let obavestenje = new obavestenja(o)
...
router.route('/dohvatiMojaObavestenja').post((req, res) => {
  let korime = req.body.korime;
  obavestenja.find({ "korime": korime }, (err, o) => {
    if (err) console.log(err)
    else res.json(o)
  })
})
...
router.route('/obrisiObavestenje').post((req, res) => {
  let id = req.body._id;
  obavestenja.deleteOne({ "_id": id }, (p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajObavestenje').post((req, res) => {
  ...
  obavestenja.updateOne({ "_id": o._id }, { "naslov": o.naslov, "sadrzaj": o.sadrzaj, "datum": o.datum, "predmeti": niz }, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiStudenta').post((req, res) => {
  let s = req.body.s;
  studenti.deleteOne({ "_id": s._id }, (p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiZaposlenog').post((req, res) => {
  let z = req.body.z;
  zaposleni.deleteOne({ "_id": z._id }, (p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajStudenta').post((req, res) => {
  let student = req.body.student;
  studenti.updateOne({ "_id": student._id }, { "korime": student.korime, "lozinka": student.lozinka, "indeks": student.indeks, "tip_studija": student.tip_studija, "status": student.status }, (err, p) => {
    if(err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiPredmet').post((req, res) => {
  let p = req.body.p;
  predmeti.deleteOne({ "_id": p._id }, (poruka) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/dohvatiAdmina').post((req, res) => {
  let korime = req.body.korime;
  let lozinka = req.body.lozinka;
  admin.findOne({"korime":korime, "lozinka":lozinka}, (err,a)=>{
    if (err) console.log(err)
    else res.json(a)
  })
})
...
router.route('/dovhatiStudenta').post((req, res) => {
  let korime = req.body.korime;
  let lozinka = req.body.lozinka;
  studenti.findOne({ "korime": korime, "lozinka": lozinka }, (err, s) => {
    if (err) console.log(err)
    else res.json(s)
  })
})
...
router.route('/promenaLozinkeStudent').post((req, res) => {
  let korime = req.body.korime;
  let lozinka = req.body.lozinka;
  studenti.updateOne({"korime": korime}, {"lozinka": lozinka, "flag": 1 }, (err, p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/promenaLozinkeZaposleni').post((req, res) => {
  let korime = req.body.korime;
  let lozinka = req.body.lozinka;
  zaposleni.updateOne({"korime": korime}, {"lozinka": lozinka, "flag": 1 }, (err, p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/dodajOpsteObavestenje').post((req, res) => {
  ...
  let o = new opsta_obavestenja(obavestenje);
  o.save().then((p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajOpsteObavestenje').post((req, res) => {
  let o = req.body.o;
  opsta_obavestenja.updateOne({ "_id": o._id }, { "naslov": o.naslov, "sadrzaj": o.sadrzaj, "kategorija": o.kategorija }, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajProjekat').post((req, res) => {
  let projekat = req.body.projekat;
  projekti.updateOne({"_id":projekat._id}, {"tema":projekat.tema, "oblast":projekat.oblast, "kontakt":projekat.kontakt, "sadrzaj":projekat.sadrzaj, "kategorija":projekat.kategorija}, (err,p)=>{
    if(err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/dohvatiNastavnika').post((req, res) => {
  let korime = req.body.korime;
  zaposleni.findOne({ "korime": korime }, (err, z) => {
    if (err) console.log(err)
    else res.json(z)
  })
})
...
router.route('/azurirajInfoPredmetZaposleni').post((req, res) => {
  ...
  predmeti.updateOne({ "_id": predmet._id }, {
...
router.route('/obrisiPredavanje').post((req, res) => {
  ...
  predmeti.updateOne({ "_id": id }, { $pull: { predavanja: {"link":link}}}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiVezbe').post((req, res) => {
  ...
  predmeti.updateOne({ "_id": id }, { $pull: { vezbe: {"link":link}}}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiIspit').post((req, res) => {
 ...
 console.log("usao u brisanje ispita back")
  predmeti.updateOne({ "_id": id }, { $pull: { ispiti: {"link":link}}}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiLab').post((req, res) => {
  let link = req.body.link;
  let id = req.body.id;
  predmeti.updateOne({ "_id": id }, { $pull: { labvezbe: {"link":link}}}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiMaterijalProjekat').post((req, res) => {
  let link = req.body.link;
  let id = req.body.id;
  predmeti.updateOne({ "_id": id }, { $pull: { projekat_materijali: {"link":link}}}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/obrisiMaterijalDomaci').post((req, res) => {
  let link = req.body.link;
  let id = req.body.id;
  predmeti.updateOne({ "_id": id }, { $pull: { domaci_materijali: {"link":link}}}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/dohvatiPredmeteNaOsnovuTipaStudija').post((req, res) => {
  let tip_studija = req.body.tip_studija;
  predmeti.find({ "tip_studija": tip_studija }, (err, p) => {
   if(err) console.log(err)
   else res.json(p)
 })
})
...
router.route('/dohvatiZahteveZaPracenje').post((req, res) => {
  let korime = req.body.korime;
  zahtevi.find({"student":korime}, (err,z)=>{
    if (err) console.log(err)
    else res.json(z)
  })
})
...
router.route('/prihvatiZahtevZaPracenje').post((req, res) => {
  let id = req.body.id;
  zahtevi.updateOne({ "_id": id }, { flag: 1 }, (err, p) => {
    res.json({"poruka":"OK"})
  })
})
...
router.route('/dodajStudentuPredmet').post((req, res) => {
  ...
  studenti.updateOne({ "korime": student }, { $push:{"predmeti":predmet}}, (err, p) => {
    if (err) console.log(err)
    else res.json({ "poruka":"OK"})
  })
})
...
router.route('/dohvatiMojeSpiskove').post((req, res) => {
  let korime = req.body.korime;
  spiskovi.find({ "zaposlen": korime }, (err, s) => {
    if (err) console.log(err)
    else res.json(s)
 })
})
...
router.route('/zatvoriSpisak').post((req, res) => {
  let id = req.body.id;
  spiskovi.updateOne({ "_id": id }, {"status":'z'}, (err, p) => {
    if (err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/dohvatiSpiskoveZaPredmet').post((req, res) => {
  let id = req.body.id;
  spiskovi.find({"predmet": id}, (err, s) => {
    if(err) console.log(err)
    else res.json(s)
  })
})
...
router.route('/azurirajAktivnostStraniceProjekat').post((req, res) => {
  let predmet = req.body.predmet;
  predmeti.updateOne({"_id":predmet._id},{"projekat_aktivna_strana":predmet.projekat_aktivna_strana}, (err,p)=>{
    if(err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajAktivnostStraniceLaboratorija').post((req, res) => {
  let predmet = req.body.predmet;
  predmeti.updateOne({"_id":predmet._id},{"lab_aktivna_strana":predmet.lab_aktivna_strana}, (err,p)=>{
    if(err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajAktivnostStraniceIspiti').post((req, res) => {
  let predmet = req.body.predmet;
  predmeti.updateOne({"_id":predmet._id},{"ispit_aktivna_strana":predmet.ispit_aktivna_strana}, (err,p)=>{
    if(err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
...
router.route('/azurirajAktivnostStraniceDomaci').post((req, res) => {
  let predmet = req.body.predmet;
  predmeti.updateOne({"_id":predmet._id},{"domaci_aktivna_strana":predmet.domaci_aktivna_strana}, (err,p)=>{
    if(err) console.log(err)
    else res.json({"poruka":"OK"})
  })
})
